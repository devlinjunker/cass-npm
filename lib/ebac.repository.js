...
EcRepository = stjs.extend(EcRepository, null, [], function(constructor, prototype) {
    ...
    prototype.autoDetectRepositoryAsync = function(success, failure) {
        ...
        if (this.selectedServer != null) {
            ...
            var url = new URL(this.selectedServer);
            if(url.parse != undefined)
              url.parse(this.selectedServer);
            hostnames.push(url.hostname);
            servicePrefixes.push(url.pathname);
            if(url.hostname.indexOf("service") == -1){
              hostnames.push(url.hostname.replace(".", ".service."));
            }
        } else {
        ...
        if(window != undefined)
          servicePrefixes.push("/" + window.location.pathname.split("/")[1] + "/api/", "/" + window.location.pathname.split("/")[1] + "/api/custom/");
        servicePrefixes.push("/", "/service/", "/api/", "/api/custom/");
        var me = this;
    ...
    prototype.autoDetectRepository = function() {
        ...
        if (window != null) servicePrefixes.push("/" + window.location.pathname.split("/")[1] + "/api/", "/" + window.location.pathname.split("/")[1] + "/api/custom/");
        servicePrefixes.push("/", "/service/", "/api/", "/api/custom/");
        for (var j = 0; j < hostnames.length; j++) {
            for (var k = 0; k < servicePrefixes.length; k++) {
                for (var i = 0; i < protocols.length; i++) {
                    console.log(protocols[i] + "//" + hostnames[j] + servicePrefixes[k].replaceAll("//", "/"));
