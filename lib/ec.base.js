...
EcRemote = stjs.extend(EcRemote, null, [], function(constructor, prototype) {
    ...
    constructor.getExpectingObject = function(server, service, success, failure) {
        ...
        if(request != undefined){
          if(p.async == false){
            console.warn("Cannot do synchronous requests in node env")
          }
          request.get(url, function(err, resp, body){
            if(err){
              EcRemote.getFailureCallback(failure)(err);
            }
            EcRemote.getSuccessJSONCallback(success, failure)({}, {},
              {status:resp.statusCode, responseText:body});
          })
        } else if ($ != null && $.ajax != null && $.ajax != undefined) {
            $.ajax(p);
        } else {
